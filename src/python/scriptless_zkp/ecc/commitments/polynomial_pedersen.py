...
class RevealedPolynomialPedersenCommitment:
    ...
    def generate_evaluation_proof(self, x: int) -> PolynomialPedersenEvaluationProof:
        ...
        y: int = int(self.committed_polynomial(x))
        ...
        # Return the NIZK polynomial evaluation proof, which encapsulates the tuple `(x, y, 𝜋)` of integers modulo `p`.
...
class PolynomialPedersenEvaluationProof:
    ...
    def verify(
        ...
        if not (0 <= self.y < sealed_commitment.field.characteristic):
            raise ValueError(
                f"Evaluation result 'y' is out of range for the field 'F_{sealed_commitment.field.characteristic}'."
            )
        ...
        right_side: ECC.EccPoint = reduce(
            ...
            [sealed_commitment.coefficient_commitments[i].commitment_point
                * pow(self.x, i, sealed_commitment.field.characteristic)
